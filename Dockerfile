...
RUN DEBIAN_FRONTEND=noninteractive apt install -y libasan6 man-db manpages\
...
# ENV ASAN_OPTIONS=strip_path_prefix=/:log_path=/tmp/asan_log/dump_log:symbolize=1:abort_on_error=1
# 关键是确保输出不被缓冲，或者直接在命令中设置
# 更简单的修复是添加 'exitcode' 或直接在脚本中设置
RUN cd libsixel-git-6a5be; all_configure
# ## Libtiff
